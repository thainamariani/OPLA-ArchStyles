...
public class NSGAII_OPLA_Multithread {
                    ...
                    for (final double mutationProbability : MUTATION_PROBABILITY) {
                        for (final String pla : PLAS) {
                            for (final String style : STYLES) {
                                createThread(populationSize, maxEvaluations, mutationProbability, pla, style);
                            }
                        }
                    }
                }
    ...
    private static synchronized void createThread(final String context) {
        //pla_population_maxevaluations_mutationprobability_style
        String[] split = context.split("_");
        final int populationSize = Integer.valueOf(split[1]);
        final int maxEvaluations = Integer.valueOf(split[2]);
        final double mutationProbability = Double.valueOf(split[3]);
        final String style = split[4];
        final String pla = ArchitectureRepository.getPlaPath(split[0]);
        createThread(populationSize, maxEvaluations, mutationProbability, pla, style, context);
    }

    private static synchronized void createThread(final int populationSize, final int maxEvaluations, final double mutationProbability, final String pla, final String style) {
        final String context = getContext(pla, String.valueOf(populationSize), String.valueOf(maxEvaluations), String.valueOf(mutationProbability), style);
        createThread(populationSize, maxEvaluations, mutationProbability, pla, style, context);
    }

    private static synchronized void createThread(final int populationSize, final int maxEvaluations, final double mutationProbability, final String pla, final String style, final String context) {
        final Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ProcessBuilder builder = new ProcessBuilder("java", "-XX:MaxPermSize=1G", "-jar", "dist/OPLA-ArchStyles.jar",
                            "" + populationSize,
                            "" + maxEvaluations,
                            "" + mutationProbability,
                            pla,
                    ...
                    final File destination = new File("experiment/" + Experiment.getPlaName(pla) + "/" + context + "/SYSTEM_OUTPUT.txt");
                    final File errorDestination = new File("experiment/" + Experiment.getPlaName(pla) + "/" + context + "/SYSTEM_ERROR.txt");
                    {
    ...
    public static synchronized String getContext(String pla, String populationSize, String maxEvaluations, String mutationProbability, String style) {
        //pla_population_maxevaluations_mutationprobability_style
        return Experiment.getPlaName(pla) + "_" + populationSize + "_" + maxEvaluations + "_" + mutationProbability + "_" + style;
    }
